...
public class CircularSeekBar extends View {
     ...
     */
    private Paint.Cap mCircleStyle = Paint.Cap.ROUND;
    ...
    /**
     * Used for enabling/disabling the touch event .
     * */
    ...
    private void initAttributes(TypedArray attrArray) {
        mCircleXRadius = attrArray.getDimension(R.styleable.CircularSeekBar_circle_x_radius, DEFAULT_CIRCLE_X_RADIUS);
        mCircleYRadius = attrArray.getDimension(R.styleable.CircularSeekBar_circle_y_radius, DEFAULT_CIRCLE_Y_RADIUS);
        mPointerStrokeWidth = attrArray.getDimension(R.styleable.CircularSeekBar_pointer_stroke_width, DEFAULT_POINTER_STROKE_WIDTH);
        mPointerHaloWidth = attrArray.getDimension(R.styleable.CircularSeekBar_pointer_halo_width, DEFAULT_POINTER_HALO_WIDTH);
        mPointerHaloBorderWidth = attrArray.getDimension(R.styleable.CircularSeekBar_pointer_halo_border_width, DEFAULT_POINTER_HALO_BORDER_WIDTH);
        mCircleStrokeWidth = attrArray.getDimension(R.styleable.CircularSeekBar_circle_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH);
        ...
        mMax = attrArray.getInt(R.styleable.CircularSeekBar_max_bar, DEFAULT_MAX);
        ...
        mLockEnabled = attrArray.getBoolean(R.styleable.CircularSeekBar_lock_enabled, DEFAULT_LOCK_ENABLED);
        mDisablePointer = attrArray.getBoolean(R.styleable.CircularSeekBar_disable_pointer, DEFAULT_DISABLE_POINTER);
        mNegativeEnabled = attrArray.getBoolean(R.styleable.CircularSeekBar_negative_enabled, DEFAULT_NEGATIVE_ENABLED);
        mIsInNegativeHalf = false;
        mDisableProgressGlow = attrArray.getBoolean(R.styleable.CircularSeekBar_disable_progress_glow, DEFAULT_DISABLE_PROGRESS_GLOW);
        mHideProgressWhenEmpty = attrArray.getBoolean(R.styleable.CircularSeekBar_hide_progress_when_empty, DEFAULT_CS_HIDE_PROGRESS_WHEN_EMPTY);
        ...
        mPointerAngle = ((360f + (attrArray.getFloat((R.styleable.CircularSeekBar_pointer_angle), DEFAULT_POINTER_ANGLE) % 360f)) % 360f);
        if (mPointerAngle == 0f) {
    ...
    private void initPaints() {
        ...
        mPointerPaint.setStrokeWidth((int) mCircleStrokeWidth);
    ...
    private void calculateProgressDegrees() {
        mProgressDegrees = mIsInNegativeHalf ? mStartAngle - mPointerPosition : mPointerPosition - mStartAngle;
    ...
    /**
     * Sets negative enabled.
     *
     * @param negativeEnabled the negative enabled
     */
     ...
     *
     * @param eventEnabled the touch event enabled
     */
    public void setTouchEventEnabled(boolean eventEnabled){
        this.mTouchEventEnabled = eventEnabled;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if(mTouchEventEnabled) {
            ...
            if (mPointerStrokeWidth < (minimumTouchTarget / 2)) {
                additionalRadius = minimumTouchTarget / 2;
            } else {
                additionalRadius = mPointerStrokeWidth;
            }
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mUserIsMovingPointer) {
                        ...
                        } else if (progressNearZero && mNegativeEnabled) {
                            if (touchOverEnd)
                                mIsInNegativeHalf = false;
                            else if (touchOverStart) {
                                mIsInNegativeHalf = true;
                            }
                        } else if (progressNearZero) {
                            if (pointerNearStart) {
    ...
    /**
     * Sets on seek bar change listener.
     *
     * @param listener the OnCircularSeekBarChangeListener
     */
    ...
    */
    ...
    /**
     * Gets pointer stroke width.
     *
     * @return the pointer stroke width
     */
